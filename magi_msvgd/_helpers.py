...
'''
Dependencies: jax, optax, sklearn, tqdm
'''
...
class MaternKernel():
    ...
    def eval(self, x, y=None):
        if y is None:
            y = x
        return jnp.array(self.skl_kernel(x, y))
...
def initialize_obs(solver):
    ...
    for d in solver.observed_components:
        ...
        interpolator = jsp.interpolate.interp1d(I_d, y_d, fill_value='extrapolate')
...
def initialize_unobs(solver):
    ...
    grad_obj = jax.value_and_grad(objective)
    x_guess_init = jnp.ones(shape=(solver.n, len(solver.unobserved_components))) * jnp.nanmean(solver.x_init)
    ...
    solver.x_init[:,solver.unobserved_components] = params[0]
...
def fit_phisigma(solver, v=2.01):
    ...
    def neglogprob(phi, sigma_d, mu_phi2, sig_phi2, y_d, I_d):
        ...
        cov = Kappa_phi.eval(I_d) + jnp.identity(I_d.shape[0])*sigma_d**2

        t1 = 1/sig_phi2**2 * (phi[1] - mu_phi2)**2
        t2 = jnp.linalg.slogdet(cov).logabsdet
        t3 = y_d.T @ jnp.linalg.pinv(cov) @ y_d
        return  0.5 * (t1 + t2 + t3)

    solver.x_init = jnp.array(solver.x_init, dtype=jnp.float64)
    for d in range(solver.D):
        ...
        idxs = jnp.linspace(1, len(zmod_effective_sq), len(zmod_effective_sq))
        ...
        if (not solver.sigmas[d]) or (jnp.isnan(solver.sigmas[d])):
            # fit sigma if it is not specified
            target = lambda phisigma: neglogprob(phisigma[:2], phisigma[2], mu_phi2, sig_phi2, y_d, I_d)
            fitted = jsp.optimize.minimize(target, x0=jnp.ones(3),
                                          bounds=[(1e-10, jnp.inf)]*3, method=method).x
            ...
            fitted = jsp.optimize.minimize(target, x0=jnp.ones(2),
                                          bounds=[(1e-10, jnp.inf)]*2, method=method).x
...
def build_matrices(solver, v=2.01):
    ...
    def build_matrices_d(I, phi1, phi2, v=v):
        ...
        l = jnp.abs(s - t); u = jnp.sqrt(2*v) * l / phi2; jnp.fill_diagonal(a=u, val=jnp.nan)
    
        # pre-compute Bessel function + derivatives
        Bv0 = jsp.special.kvp(v=v, z=u, n=0)
        Bv1 = jsp.special.kvp(v=v, z=u, n=1)
        Bv2 = jsp.special.kvp(v=v, z=u, n=2)
        ...
        jnp.fill_diagonal(Kappa, val=phi1) # behavior as |s-t| \to 0^+
        ...
        jnp.fill_diagonal(p_Kappa, val=0.0) # behavior as |s-t| \to 0^+
        ...
        jnp.fill_diagonal(Kappa_pp, val=v*phi1/( (phi2 ** 2) * (v-1) )) # behavior as |s-t| \to 0^+
    ...
    solver.C_invs, solver.ms, solver.K_invs = [jnp.array(mats) for mats in \
