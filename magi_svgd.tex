...
doubles as a design rule, and this is the third thing worth keeping:

\begin{equation}
  \text{profiled SVGD needs } \ln K \gtrsim p, \text{ i.e. } K \gtrsim e^{p},
\end{equation}

about $20$ particles at $p = 3$ and $150$ at $p = 5$, confirmed over seven values of $p$ as
$\ln K_{\mathrm{crit}} = 0.86p + 1.45$ with the rate holding to $8$--$10\%$. Below it the parameter
ensemble collapses exactly as the joint one does.
...
\item \textbf{$K \gtrsim e^{p}$}, for anyone running SVGD on a low-dimensional marginal.
\end{enumerate}
