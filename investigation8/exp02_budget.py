...
for name in SYS:
    ...
    rec = {"h0": h0, "mult": MULT, "floor": flk,
           "floors": dict(stiff_var=sv_fl, ks=ks_fl, wb=wb_fl), "starts": {}}
    print(f'   floors: energy {flk:.4f}  stiffvar {sv_fl:.3f}  KS {ks_fl:.4f}', flush=True)
    for slab, Xs in STARTS.items():
        ...
        print(f'{"iter":>8} {"energy":>9} {"x floor":>8} {"stiffvar":>9} {"KS/flr":>7} '
              f'{"whsd":>7} {"SteinR":>8} {"thErr":>8} {"sec":>7}', flush=True)
        t0 = time.time()
        ...
        for it, Q in hist:
            r = dict(it=it, energy=S.energy(Q), whsd=S.mahalanobis_sd(Q),
                     ...
                     band=S.band_profile(Q).tolist(), **MM.score(S, Q, ref))
            traj.append(r)
            print(f'{it:>8} {r["energy"]:>9.4f} {r["energy"]/flk:>8.2f} '
                  f'{r["stiff_var"]:>9.4f} {r["ks"]/ks_fl:>7.2f} {r["whsd"]:>7.3f} '
