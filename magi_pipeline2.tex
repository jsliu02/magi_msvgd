...
the mode alone is $1.80$ out. It costs four to six seconds against roughly two hundred and up for
a reference chain, and it reports its own reliability without one.
...
benchmark that has a usable reference, in four to six seconds, and reporting its reliability
without a reference.
...
requires no sampler and costs four to five seconds per system on a V100, almost all of it XLA
compilation: a second call on the same model returns in $0.1$ to $0.3$\,s. The persistent
compilation cache makes that a once-per-machine cost rather than a once-per-process one.
...
The curvature is a subtler matter, and we previously drew the wrong conclusion from the right
fact. The second derivative of $-U(\theta, X^\star(\theta))$ at the joint mode is exactly the Schur
complement $(\Sigma_{\theta\theta})^{-1}$, and the determinant term contributes negligibly there:
measured by finite differences at the joint mode, the profiled curvature matches the Schur
complement to within $3\%$ on all four systems. We took that to mean the finite-difference Hessian
was redundant.

It is not, because the profiled posterior is not evaluated at the joint mode. The profile mode sits
one to one and a half Laplace standard deviations away, and the curvature is not constant over that
distance:

\begin{center}\small
\begin{tabular}{lccr}
\toprule
& \multicolumn{2}{c}{profiled curvature $/$ Schur complement, per parameter}\\
system & at the joint mode & at the profile mode & mode shift\\
\midrule
hiv    & 1.000 -- 1.000 & 1.000 -- 1.002 & 0.16\,sd\\
fn     & 0.981 -- 1.019 & 1.039 -- 1.107 & 1.07\,sd\\
lorenz & 0.979 -- 0.994 & \textbf{0.588 -- 2.088} & 1.42\,sd\\
hes1   & 0.969 -- 1.023 & \textbf{0.248 -- 4.476} & 1.43\,sd\\
\bottomrule
\end{tabular}
\end{center}

On HIV, whose profile mode barely moves, the joint Laplace curvature would serve. On Lorenz and
Hes1 it is wrong by factors of two and four, which no $1.3\times$ proposal inflation absorbs. The
ordering follows $\operatorname{cond}(M)$ from Section~\ref{sec:diagnosis}: the systems whose
curvature varies over the posterior are the ones where measuring it where it is actually needed
matters. So the finite-difference Hessian stays, and Section~\ref{sec:stencil}'s care over the step
...
The gate fires on exactly one system, Hes1, and it fires there for some Sobol seeds and not
others---in double precision as well as single, with the effective sample size ranging from
$2.3\%$ to $21.1\%$ over four seeds (Section~\ref{sec:precision}). That is what a gate is for.
Hes1 is the system whose geometry Section~\ref{sec:funnel} shows to be a funnel, whose reference
chain does not converge, and whose profiled fit is not reproducible; the other three pass at
$62$ -- $77\%$ regardless of seed or precision. The case that used to justify the gate---a
posterior where nothing was identified and profiling made matters worse---was an artifact of the
broken hyperparameters and we withdrew it. This is the replacement, and it is a better one,
because the failure it catches is a failure of the method rather than of the model.
...
\caption{Profiled MAGI posterior, $4$ -- $6$\,s on a V100}
\label{alg:profiled}
\end{algorithm}
...
Section~\ref{sec:gp}. Errors are float64; the times in the last column are the recommended
configuration, float32 on a V100, where the errors agree to a percent (Section~\ref{sec:precision}). On FitzHugh--Nagumo, HIV and Lorenz the profiled posterior is at or
...
first two. The costs in the last column---four to six seconds, of which most is compilation---are against
...
Hes1 looks like an exception and is really a different problem. We had reported that single
precision takes its effective sample size from $21\%$ to $2.1\%$; that compared two runs at one
Sobol seed, and Hes1's effective sample size is not reproducible across seeds in \emph{either}
precision:

\begin{center}\small
\begin{tabular}{llrrrrr}
\toprule
system & & \multicolumn{4}{c}{ESS by Sobol seed} & spread\\
\midrule
fn, hiv, lorenz & f32, f64 & \multicolumn{4}{c}{62 -- 77\% throughout} & $\le 1.1\times$\\
hes1 & f64 & 21.1\% & 14.0\% & 2.3\% & 18.4\% & $\mathbf{9.1\times}$\\
hes1 & f32 & 0.6\% & 13.2\% & 8.4\% & 12.2\% & $\mathbf{23\times}$\\
\bottomrule
\end{tabular}
\end{center}

So the gate fires on Hes1 for some seeds and not others, in double precision as well as single,
and any single number quoted for it---including the $0.2163$ in Table~\ref{tab:results}---is one
draw from a wide distribution. That is a better argument for the gate than the one we had: it
catches a fit that genuinely is not reliable, on the one system whose geometry
(Section~\ref{sec:funnel}) says it should not be. Everywhere else the two precisions agree to a
percent and single precision is the better choice.

\paragraph{A guard that did not cover enough.} Setting \texttt{default\_matmul\_precision} to
\texttt{highest} on the products that form $J^\top J$ is not sufficient on tensor-core hardware.
The residual and the Jacobian are themselves assembled from einsums against the GP factors, which
XLA is equally free to run in TF32, and reduced precision there corrupts $J$ before $J^\top J$ is
ever formed. Unguarded, HIV in float32 on a V100 reports an effective sample size of $35.6\%$
against $71.2\%$ for the same fit on CPU; guarded, it reports $74.6\%$, and costs no measurable
time. The failure is silent and shows up only when one configuration is run on two devices, which
is a reason to do that at least once.
...
\emph{Profiling buys nothing near the joint mode}, for the structural reason in
Section~\ref{sec:profiled} -- but that is not an argument against computing the profiled curvature,
as we had read it. It is only a statement about a point the method does not use.
