...
\midrule
iterations & 17 & 15 & 16 & 42 & 4 & 4 & 31 & 37\\
$\lambda_{\mathrm{N}}$ & 6.3e-4 & 1.7e-3 & 2.0e-4 & 2.2e-4 & 6.7e-6 & 5.7e-4 & 8.8e-4 & 3.9e-3\\
$\|\nabla\log p\|$ & 2.6e-3 & 1.0e-2 & 1.1e-2 & 8.4e-2 & 3.7e-4 & 3.6e-2 & 5.1e-4 & 5.2e-3\\
\bottomrule
\end{tabular}
\end{center}
...
($8.4\times10^{-2}$) looks like the worst-converged run in the suite by two orders of magnitude,
while its decrement, $2.2\times10^{-4}$, is the second best. The gradient norm is reporting the
...
its three components was NaN before the first step. The cause was a fixed $10^{-12}$ relative
ridge on the Cholesky---about $4500$ times float64's epsilon, and five orders \emph{below}
float32's, so in single precision it was not representable against the diagonal it was meant to
lift. Once the hyperparameters were fitted correctly HIV's $C^{-1}$ had condition $1.3\times10^{9}$
and that was enough to expose it. The ridge now follows the dtype. It is the same error as
everywhere else in this paper, in its purest form: a constant that had been calibrated against one
scale and was being applied to another.

\begin{remark}[A recurring theme]
This is the fifth place in this paper where a quantity with units was being compared against an
absolute threshold, after the conditioning of the normal equations (Section~\ref{sec:solver}), the
detection of flat directions (Section~\ref{sec:diagnosis}), the finite-difference step
(Section~\ref{sec:stencil}) and the Cholesky ridge just above. Two more follow: the agreement test
between restarts (Section~\ref{sec:diagnosis}) and, worst of all, the starting point of the
hyperparameter optimiser (Section~\ref{sec:gp}). In each case the fix is to divide by the relevant
scale---the diagonal of $A$, the diagonal of $H$, the posterior standard deviation, machine
epsilon, the data's own variance---and in each case the unscaled version worked on the benchmark
it was developed on and failed elsewhere.
\end{remark}
...
first order, and on Hes1 the log importance weights then span $8{,}400$ nats with
$\mathrm{ESS} = 0.6\%$. Since $p$ is small, the profiled marginal's own mode and curvature are
directly computable---a Newton solve in $p$ dimensions by central differences through
...
$X_{\mathrm{MAP}}$ lets the inner iteration count fall from six to two at unchanged accuracy
(Table~\ref{tab:predictor}).
...
\midrule
from $X_{\mathrm{MAP}}$ & 0.0437 / 5.8\,s & 0.0065 / 6.8\,s & 0.0056 / 8.7\,s\\
\textbf{from the predictor} & \textbf{0.0059 / 4.5\,s} & 0.0063 / 6.1\,s & 0.0065 / 9.1\,s\\
\bottomrule
\end{tabular}
\caption{Largest parameter error in reference standard deviations, and wall clock, on
FitzHugh--Nagumo. The chain-to-chain floor is $0.0120$.}
\label{tab:predictor}
\end{table}
...
\midrule
fn, float64 & $\theta_1$ & 1.145 & 1.145 & 1.150 & 1.163 & 1.217 & 1.425 & ---\\
fn, float32 & $\theta_1$ & \textbf{0.439} & 1.147 & 1.146 & 1.162 & 1.217 & 1.425 & ---\\
hes1, both  & $\theta_5$ & 169.4 & 129.0 & \textbf{34.5} & 13.0 & 4.0 & 1.3 & ---\\
lorenz, both& $\theta_2$ & 0.505 & 0.504 & 0.482 & \textbf{---} & --- & --- & ---\\
\bottomrule
\end{tabular}
\caption{Diagonal curvature of the profiled log-density, scaled so a Gaussian profile would read
$1.0$ at every $h$. Three different failure modes bound the step: round-off at small $h$ in single
precision (fn), truncation at large $h$ where the profile is non-quadratic (hes1), and outright
failure of the inner solve (lorenz). The usable windows do not overlap---fn in float32 needs
$h \gtrsim 0.2$ while hes1 needs $h \lesssim 0.05$---so no constant serves both.}
\label{tab:stencil}
\end{table}
...
\emph{Agreement must be across the run, not between neighbours.} On FitzHugh--Nagumo every
consecutive change stays under $5\%$ from $h = 0.05$ out to $3.2$ while the curvature drifts from
$1.145$ to $1.217$; small steps accumulate, and a neighbour test walks steadily off the plateau it
is meant to find.
...
on either side. Taking the top is measurably worse: the run is located from diagonal probes while
the Newton also needs off-diagonal ones at $\sqrt2$ times the displacement, and on Lorenz the top
of the run made those non-finite and cost three quarters of the effective sample size.

\emph{A failed probe is not agreement.} Judging coherence on whichever parameters happen to be
finite silently drops those that failed. On Lorenz the third parameter stops resolving beyond
$h = 0.8$, and ignoring that extended the apparent plateau to $3.2$---the same collapse again, by a
...
selecting rather than setting them.
...
$\mathrm{ESS} = 2.3\%$. The case that used to justify it---a
...
hyperparameter fit its profiled error was $0.34$ against a floor of $4.14$, and we attributed the
...
$21\%$ to $2.3\%$ and the gate declines. This is not the stencil---both precisions pick a step
inside the same plateau---but the funnel of Section~\ref{sec:funnel} interacting with the ridge the
Cholesky needs in float32, which is $4.9\times10^{-4}$ relative against $9\times10^{-13}$ in
float64. On a posterior whose curvature varies by six orders of magnitude that ridge is not
uniformly negligible. The honest summary is therefore narrower than we first wrote it: use single
precision, and expect the gate to catch the case where it is not enough.
...
\midrule
fn     & 0.1  & 1.6  & 0.01--1.6  & 0.4--6.4$^\ast$  & 75.7\% & 75.8\%\\
hiv    & 0.2  & 1.6  & 0.01--6.4$^\ast$ & 0.4--6.4$^\ast$ & 63.4\% & 79.4\%\\
lorenz & 0.1  & 0.4  & 0.01--0.8  & 0.2--0.8   & 67.5\% & 72.4\%\\
hes1   & 0.05 & 0.1  & 0.01--0.4  & 0.02--0.4  & 21.1\% & \textbf{2.3\%}\\
\bottomrule
\end{tabular}
...
A pattern runs through all of them. Seven separate quantities in this pipeline were being compared
...
agreement test between restarts, the ridge added before a Cholesky, and the starting point of the
hyperparameter optimiser. Each worked on the problem it was developed on and failed on another,
and in every case the repair was the same---divide by the scale that the quantity is measured in,
and the threshold becomes a statement about the posterior rather than about the units. It is a
dull principle, and it accounted for more of the difficulty here than any of the approximation
theory. The last of the seven is the one to remember, because it did not announce itself as a
numerical problem at all: it produced a posterior that was internally consistent, passed every
certificate we had, and answered a different question than the one asked.
